        ...
        try:
            ...
            if run and run.get('status') == 'SUCCEEDED':
                ...
                if leads:
                    ...
                    # Mapeamento de colunas desejadas (Chave no JSON: Nome na Tabela)
                    cols_map = {
                        "title": "Título",
                        "price": "Preço",
                        "rooms": "Quartos",
                    ...
                    # Filtra apenas as colunas que realmente vieram no resultado
                    existing_cols = [c for c in cols_map.keys() if c in df.columns]
                    
                    if existing_cols:
                        df_display = df[existing_cols].rename(columns=cols_map)
                        st.dataframe(df_display, use_container_width=True)
                    else:
                        st.warning("Os dados vieram em um formato inesperado, mas você ainda pode baixar o arquivo completo abaixo.")
                        st.write(df.head()) # Mostra o que veio para debug
                    
                    # Download Excel (Sempre com todos os dados originais)
                    output = BytesIO()
                    with pd.ExcelWriter(output, engine='openpyxl') as writer:
                        df.to_excel(writer, index=False)
                    st.download_button("📊 Baixar Excel Completo", output.getvalue(), f"leads_{loc_slug}.xlsx", use_container_width=True)
                else:
...
st.markdown("---")
st.caption("Desenvolvido por Manus AI | Captação Inteligente v4")
